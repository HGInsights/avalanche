defmodule Avalanche.Steps.GetPartitions do
  ...
  def get_partitions({request, %Req.Response{body: ""} = response}) do
    {request, response}
  end

  def get_partitions({request, %Req.Response{status: 200, body: %{"resultSetMetaData" => metadata} = body} = response}) do
  ...
  defp error_response(error) do
    ...
    %Req.Response{status: 500, body: nil}
  end

  # coveralls-ignore-stop

  defp reduce_responses(%Req.Response{} = response, data, partition_responses) do
